...
export type LessonRelationshipType =
  "derives-from" | "corrects" | "extends" | "contradicts" | "see-also";

export type LessonRelationships = Partial<
  Record<LessonRelationshipType, string[]>
>;
...
export type VaultTransferClassification =
  | "new"
  | "identical"
  | "already_present"
  | "same_id"
...
export type VaultDangerOperation =
  "empty" | "reset" | "delete" | "merge_delete_source";
...
export interface CandidateProvenance {
  ...
  supporting_evidence: CandidateSourceEvidence[];
...
export const api = {
  ...
  settingsRuntime: () => request<SettingsRuntimeResponse>("/settings/runtime"),
  ...
  markNotDuplicates: (
    pair: Pick<
      DuplicatePair,
      "left_id" | "right_id" | "left_fingerprint" | "right_fingerprint"
    >,
  ) =>
  ...
  listLessons: (limit = 50, lifecycleIn?: LessonLifecycleState[] | null) => {
    const params = new URLSearchParams({ limit: String(limit) });
  ...
  exportSearch: async (
    ...
    const resp = await fetch(
      `/export/search?format=${encodeURIComponent(format)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
  ...
  editorSuggest: (text: string, topK = 5, minScore = 0.1, explain = false) =>
    request<SimilarResponse>(
      `/editor/suggest?explain=${explain ? "true" : "false"}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, top_k: topK, min_score: minScore }),
  ...
  trainTopic: () => request<TrainResponse>("/train/topic", { method: "POST" }),
  ...
  previewVaultRestore: (id: string, artifact: Blob) =>
    request<VaultRestorePreview>(
      `/vaults/${encodeURIComponent(id)}/restore/preview`,
      {
        method: "POST",
  ...
  restoreVaultSnapshot: (id: string, artifact: Blob, planDigest: string) =>
    request<VaultRestoreResponse>(
      `/vaults/${encodeURIComponent(id)}/restore?plan_digest=${encodeURIComponent(planDigest)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/zip" },
        body: artifact,
  ...
  vaultTransferSourceLessons: (id: string) =>
    request<VaultTransferSourceLesson[]>(
      `/vault-transfers/sources/${encodeURIComponent(id)}/lessons`,
    ),
  ...
  createVault: (name: string, path: string) =>
    request<ManagedVault>("/vaults/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, path }),
    }),

  registerVault: (name: string, path: string) =>
    request<ManagedVault>("/vaults/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, path }),
    }),
  ...
  renameVault: (id: string, name: string) =>
    request<ManagedVault>(`/vaults/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
    }),

  activateVault: (id: string) =>
    request<VaultStatusResponse>(`/vaults/${encodeURIComponent(id)}/activate`, {
      method: "POST",
    }),

  removeVault: (id: string) =>
    request<void>(`/vaults/${encodeURIComponent(id)}`, { method: "DELETE" }),
  ...
  markLessonReviewed: (id: string, expectedRevision: string) =>
    request<LessonReviewResponse>(`/lessons/${encodeURIComponent(id)}/review`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  bulkDeleteLessons: (lessonIds: string[]) =>
    request<BulkLessonDeleteResponse>("/lessons/bulk-delete", {
      ...
      body: JSON.stringify({
        lesson_ids: lessonIds,
      } satisfies BulkLessonDeleteRequest),
    }),

  opsRefresh: (train = true) =>
    request<OpsRefreshResponse>(
      `/ops/refresh?train=${train ? "true" : "false"}`,
      {
        method: "POST",
      },
    ),
  ...
  statsTimeline: (groupBy: "year" | "month" | "topic" = "month") =>
    request<TimelineResponse>(
      `/stats/timeline?group_by=${encodeURIComponent(groupBy)}`,
    ),
  ...
  previewSemanticIngestion: (body: RawSourceInput) =>
    request<IngestionResult>("/api/v1/tritalele/semantic/preview", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),

  stageSemanticIngestion: (body: RawSourceInput) =>
    request<IngestionResult>("/api/v1/tritalele/semantic/stage", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),
  ...
  listCandidates: (filters: CandidateFilters = {}) => {
    ...
    if (filters.chunk_index != null)
      query.set("chunk_index", String(filters.chunk_index));
    const suffix = query.size ? `?${query.toString()}` : "";
    return request<CandidateListResponse>(
      `/api/v1/tritalele/candidates${suffix}`,
    );
  },

  getCandidate: (candidateId: string) =>
    request<Candidate>(
      `/api/v1/tritalele/candidates/${encodeURIComponent(candidateId)}`,
    ),
